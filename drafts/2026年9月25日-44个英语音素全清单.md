...
## 六、不用自己部署：打开浏览器就能点读的在线网站

SoundCode 44 要本地跑、还只免费一半，用着有门槛。叶扬于是把上一轮调研里的项目挨个查了一遍，问的是同一个问题：有没有已经部署在线上、点开网页就能点读、免费、最好还开源的？答案是有，而且不止一个。这一节按贴合「教小朋友认音素」的程度排序。

### 6.1 音标有谱：音素和单词逐行对照，各带发音

地址 https://english-ipa.netlify.app ，部署在 Netlify 上，免费打开即用，国内可以直连。

它的设计正好踩在「音素和单词对照着学」这个需求上。选中一个字母，页面列出这个字母在英语里能对应的全部音素，每个音素一栏，下面排四个例词，音素和每个例词旁边都有独立播放按钮，例词还标了 IPA。比如选中 A，会看到 /eɪ/（name、cake）、/æ/（cat、apple）、/ɑː/（car、father）、/ə/（about、banana）五种读音并排，点哪个读哪个。页头还能循环播放、切换展示符号，底部有元音辅音、发音方式的筛选标签。

![音标有谱：选中字母 A，列出 /eɪ/ /æ/ /ɑː/ /ə/ 四个音素，各配例词与播放按钮，例词标注 IPA](https://raw.githubusercontent.com/yeyangchen2009/yeyangchen2009.github.io/main/docs/screenshots/sc44-40-online-ipa.png)

这是本轮最适合带小朋友逐个音素过的一个。需要说明的是它按 48 音标组织、例词走美式发音，和本文的英式 44 音素分类略有出入，但声音本体一致，符号对照着看即可。仓库 `resetsix/english-ipa` 代码公开，但没有附 LICENSE 文件。

### 6.2 自然拼读词典在线版：规则库加 TTS，搜词看拆解

上一篇调研里断层领先的 phonics-app 也有官方在线版：https://phonics.thetruetao.com/zh ，免费、国内直连，不用克隆仓库。

首页三个入口：搜索任意单词查看音素拆解，卡片模式背单词，发音规则百科。背后是那份 2.25 MB、16 类 100 多条规则的规则库，发音走 TTS，支持中、英、日、韩、越南语五种界面。

![自然拼读词典在线版：搜索框、背单词、发音规则百科三个入口，支持多语言界面](https://raw.githubusercontent.com/yeyangchen2009/yeyangchen2009.github.io/main/docs/screenshots/sc44-41-phonics-online.png)

它的强项是规则系统和词汇量，适合在认完音素之后查词、按规则系统过一遍。同样如实标注：`cocojojo5213/phonics-app` 仓库目前没有附 LICENSE 文件。

### 6.3 Buzzphonics：英国本土 Phase 体系，免费还带游戏

地址 https://www.buzzphonics.com ，免费在线，国内可以直连。它严格按英国小学的 Phase 2、Phase 3、Phase 5 分阶段：Phase 2 是 19 个字母音，Phase 3 是 25 个字母组合，Phase 5 是 22 个声音，另有一个游戏入口。

![Buzzphonics 首页：Phase 2、Phase 3、Phase 5 与 Games 四个入口，按英国小学阶段划分](https://raw.githubusercontent.com/yeyangchen2009/yeyangchen2009.github.io/main/docs/screenshots/sc44-42-buzzphonics.png)

想完全照英国孩子的顺序学，选这个最对口，低龄小朋友还能靠游戏环节坐住。仓库 `hellodeborahuk/buzzphonics` 是 MIT 许可证，这是本轮唯一一个既免费在线、又明确开源、体系还完整的项目。

### 6.4 纯点读表：两张，手机上随手点

如果只想要一张符号表、点一下读一个音，还有两个免费在线的单页站点：

- zhen-ke 的 KK 音标表 https://zhen-ke.github.io/phonetic-Symbols/ ，美式 KK 体系，每个符号带播放按钮，按单元音、双元音、辅音分区；
- boommanpro 的英语音标学习 https://boommanpro.github.io/phonetic_symbol_web/ ，48 DJ 体系，点格子发音、双击进详情，仓库是 MIT 许可证。

![KK 音标点读表：每个音标一个播放按钮，点击即读](https://raw.githubusercontent.com/yeyangchen2009/yeyangchen2009.github.io/main/docs/screenshots/sc44-43-kk-chart.png)

这两个页面都部署在 GitHub Pages，免费、国内直连，适合手机上随手点着复习。

### 6.5 一张表汇总，外加两个坑

| 网站 | 体系 | 免费 | 开源许可 | 国内直连 |
|---|---|---|---|---|
| 音标有谱 | 48 音标，美式例词 | 是 | 代码公开，无 LICENSE | 可 |
| 自然拼读词典 | phonics 规则体系 | 是 | 代码公开，无 LICENSE | 可 |
| Buzzphonics | 英国 Phase 2/3/5 | 是 | MIT | 可 |
| zhen-ke 点读表 | 美式 KK | 是 | 代码公开，无 LICENSE | 可 |
| boommanpro 点读表 | 48 DJ | 是 | MIT | 可 |
| SoundCode 44 | 英式 44 音素 | 半免费 | 无 LICENSE | 域名直连失败 |

两个实测踩到的坑。第一，SoundCode 44 和另一款发音应用的 Vercel 域名在国内网络下直连打不开（curl 直接超时），GitHub Pages、Netlify 和自有域名的几个站点正常；不想折腾代理就避开 Vercel 那两个。第二，浏览器有音频自动播放安全策略，这类点读站第一次进入时往往要先在页面任意位置点一下，之后发音才会响，属于浏览器统一限制，不是网站故障。

给这套需求收个底：**不用本地部署，免费在线点读完全够用。** 叶扬设想的带学路径是，用 Buzzphonics 或音标有谱把声音逐个过一遍并跟读，遇到具体单词用自然拼读词典查拆解，平时手机上留点读表复习。

## 七、正面回答：掌握 44 个音素，读英语就很快吗
...
- **Netlify**：一个静态网站托管平台，把网页文件传上去就能得到一个在线网址，有免费额度；同类平台还有 GitHub Pages、Vercel。
- **LICENSE / 开源授权**：代码公开可见不等于开源；仓库里没有 LICENSE 文件时，默认版权全保留，他人无权随意复用。

## 参考链接

- 音标有谱（在线点读）：https://english-ipa.netlify.app ，仓库 https://github.com/resetsix/english-ipa
- 自然拼读词典在线版：https://phonics.thetruetao.com/zh ，仓库 https://github.com/cocojojo5213/phonics-app
- Buzzphonics（在线，MIT）：https://www.buzzphonics.com ，仓库 https://github.com/hellodeborahuk/buzzphonics
- KK 音标点读表：https://zhen-ke.github.io/phonetic-Symbols/
- 48 DJ 音标点读（MIT）：https://boommanpro.github.io/phonetic_symbol_web/
